 ...
 * 
 * Creates all 17 tables for the TicketSpace platform.
 * Run via: php migrations/migrate.php
 */
...
try {
    ...
    $sql = <<<SQL
-- Enable foreign keys (InnoDB default, but good practice to ensure)
...
SQL;
    ...
    echo "Inserting seed data (roles & settings)...\n";

    // Seed Roles
    $roles = [
        ['slug' => 'super_admin', 'name' => 'Super Admin', 'desc' => 'Full platform access'],
        ['slug' => 'admin', 'name' => 'Admin', 'desc' => 'Platform management'],
        ['slug' => 'host', 'name' => 'Event Host', 'desc' => 'Can create and manage events'],
        ['slug' => 'staff', 'name' => 'Event Staff', 'desc' => 'Can scan tickets at events'],
        ['slug' => 'buyer', 'name' => 'Ticket Buyer', 'desc' => 'Can browse and buy tickets'],
    ];

    $stmt = $db->prepare('INSERT IGNORE INTO roles (slug, name, description) VALUES (:slug, :name, :desc)');
    foreach ($roles as $role) {
        $stmt->execute([':slug' => $role['slug'], ':name' => $role['name'], ':desc' => $role['desc']]);
    }

    // Seed Default Settings
    $settings = [
        ['key' => 'site_name', 'value' => 'TicketSpace', 'type' => 'string'],
        ['key' => 'maintenance_mode', 'value' => '0', 'type' => 'boolean'],
        ['key' => 'allow_registrations', 'value' => '1', 'type' => 'boolean'],
        ['key' => 'default_currency', 'value' => 'EUR', 'type' => 'string'],
        ['key' => 'platform_fee_fixed', 'value' => '0.50', 'type' => 'float'],
        ['key' => 'platform_fee_percent', 'value' => '2.0', 'type' => 'float'],
    ];

    $stmt = $db->prepare('INSERT IGNORE INTO app_settings (setting_key, setting_value, type) VALUES (:key, :val, :type)');
    foreach ($settings as $setting) {
        $stmt->execute([':key' => $setting['key'], ':val' => $setting['value'], ':type' => $setting['type']]);
    }
